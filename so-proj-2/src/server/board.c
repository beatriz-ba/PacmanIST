...
int move_pacman(board_t *board, int pacman_index, command_t *command) {
    ...
    // Calcula os índices lineares da nova posição e da posição atual no
    // tabuleiro
    int new_index = get_board_index(board, new_x, new_y);
    int old_index = get_board_index(board, pac->pos_x, pac->pos_y);

    // Bloqueio dos mutexes (Locks)
    // Para evitar deadlocks, sempre bloqueamos os mutexes em ordem crescente de
    // índice. Isso garante uma hierarquia de recursos consistente.
    ...
    // Atualiza o tabuleiro: remove o Pacman da posição antiga
    board->board[old_index].content = ' ';

    // Atualiza as coordenadas internas do Pacman
    pac->pos_x = new_x;
    pac->pos_y = new_y;

    // Coloca o Pacman na nova posição do tabuleiro
    board->board[new_index].content = 'P';

    // Desbloqueia os mutexes das posições envolvidas
    // A ordem de desbloqueio segue a mesma lógica do bloqueio (menor índice
    // primeiro)
    if (old_index < new_index) {
...
// Rótulo para tratamento de movimento inválido (ex: parede)
move_pacman_invalid:
    // Garante que os mutexes sejam liberados antes de retornar
    if (old_index < new_index) {
        pthread_mutex_unlock(&board->board[old_index].lock);
        pthread_mutex_unlock(&board->board[new_index].lock);
    } else {
...
// Rótulo para quando o Pacman morre (ex: colisão com fantasma)
move_pacman_dead:
    // Garante que os mutexes sejam liberados antes de retornar
    if (old_index < new_index) {
        pthread_mutex_unlock(&board->board[old_index].lock);
...
int move_ghost_charged(board_t *board, int ghost_index, char direction) {
    ...
    ghost->charged = 0; // Descarrega o fantasma após o movimento
    ...
    case 'W': // Cima
        ...
        // Bloqueia toda a coluna de 0 até y (posição atual)
        // Isso garante acesso exclusivo a todo o caminho potencial do movimento
        // "charged" A ordem crescente de índices (i * width + x) previne
        // deadlocks
        for (int i = 0; i <= y; i++) {
            pthread_mutex_lock(&board->board[i * board->width + x].lock);
        }

        new_y = 0; // Assume que vai até o topo se não houver colisão
        // Verifica colisão de baixo para cima (do atual para o topo)
        for (int i = y - 1; i >= 0; i--) {
            char target_content = board->board[i * board->width + x].content;
            if (target_content == 'W' || target_content == 'M') {
                new_y = i + 1; // Para antes da colisão
                result = VALID_MOVE;
                break;
            } else if (target_content == 'P') {
                new_y = i; // Colide com Pacman
    ...
    case 'S': // Baixo
        ...
        // Bloqueia da posição atual até o fim da coluna
        for (int i = y; i < board->height; i++) {
            pthread_mutex_lock(&board->board[i * board->width + x].lock);
        }

        new_y = board->height - 1; // Assume que vai até o fundo
        // Verifica colisão de cima para baixo
    ...
    case 'A': // Esquerda
        ...
        // Bloqueia a linha da esquerda (0) até a posição atual (x)
        for (int j = 0; j <= x; j++) {
            pthread_mutex_lock(&board->board[y * board->width + j].lock);
        }

        new_x = 0; // Assume que vai até a esquerda
        // Verifica colisão da direita para a esquerda
        for (int j = x - 1; j >= 0; j--) {
            char target_content = board->board[y * board->width + j].content;
            if (target_content == 'W' || target_content == 'M') {
                new_x = j + 1; // Para antes da colisão
                result = VALID_MOVE;
                break;
            } else if (target_content == 'P') {
                new_x = j;
    ...
    case 'D': // Direita
        ...
        // Bloqueia da posição atual (x) até o fim da linha
        for (int j = x; j < board->width; j++) {
            pthread_mutex_lock(&board->board[y * board->width + j].lock);
        }

        new_x = board->width - 1; // Assume que vai até a direita
        // Verifica colisão da esquerda para a direita
    ...
    // Atualiza o tabuleiro - limpa posição antiga (ou restaura dot)
    board->board[y * board->width + x].content = ' ';

    // Atualiza posição do fantasma na estrutura
    ghost->pos_x = new_x;
    ghost->pos_y = new_y;

    // Atualiza o tabuleiro - define nova posição
...
int move_ghost(board_t *board, int ghost_index, command_t *command) {
    ...
    for (int i = 0; i < board->n_pacmans; i++) {
        if (board->pacmans[i].alive && board->pacmans[i].pos_x == new_x &&
            board->pacmans[i].pos_y == new_y) {
            board->pacmans[i].alive = 0;
            kill_pacman(board, i);
            result = DEAD_PACMAN;
            break;
        }
...
void open_debug_file(char *filename) { debugfile = fopen(filename, "w"); }

void close_debug_file() { fclose(debugfile); }
