...
typedef struct {
    ...
    pacman_t *pacmans;  // array containing every pacman in the board to iterate
                        // through when processing
    int n_ghosts;       // number of ghosts in the board
    ghost_t *ghosts;    // array containing every ghost in the board to iterate
                        // through when processing
    char level_name[256]; // name for the level file to keep track of which will
                          // be the next
...
/*Move pacman/monster in a certain direction on the board must check for
boundaries, walls and other monsters Maybe do 1 function for pacman and 1 for
monsters if required Maybe do 1 function for each direction
*/
...
*/
int load_level(board_t *board, char *filename, char *dirname,
