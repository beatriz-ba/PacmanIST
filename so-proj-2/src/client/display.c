...
void draw_board_client(Board board) {
    ...
    } else if (board.victory && !board.game_over) {
        mvprintw(1, 0, " VICTORY ");
    } else if (board.game_over &&
               board
                   .victory) { // Quando já não há mais níveis e o jogador vence
        mvprintw(1, 0, " END OF GAME ");
    } else {
...
// Does exaclty the same as draw board but stores the output in a string instead
// of printing it
